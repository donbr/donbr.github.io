...
if [ "${GIT_MISSING}" != "true" ]; then
  ...
  git checkout -b main
  git add .
  git commit -m "Initial commit: Bootstrap lightweight React app"
else
...
A lightweight React application bootstrapped with Vite, TypeScript, and Tailwind CSS.
...
- Component architecture based on shadcn/ui design patterns
