...
interface GdeltRecord {
  ...
  v2: {
    enhancedThemes: Array<{ theme: string; offset: number }>;
    enhancedPersons: Array<{ name: string; offset: number }>;
    enhancedOrganizations: Array<{ name: string; offset: number }>;
  };
  v2_1: {
    quotations: Array<{ quote: string; offset: number }>;
    allNames: string[];
    amounts: Array<{ value: string; offset: number }>;
  };
}
...
export const ListCard: React.FC<ListCardProps> = ({ title, items, type = "simple" }) => {
...
const GdeltRecordViewer: React.FC = () => {
  const [records, setRecords] = useState<Record<string, GdeltRecord>>({});
  const [selectedRecord, setSelectedRecord] = useState<string | null>(null);
  const [activeTab, setActiveTab] = useState<'metadata' | 'themes' | 'entities' | 'tone' | 'map'>('metadata');
  const [activeVersion, setActiveVersion] = useState<'v1' | 'v2' | 'v2_1'>('v1');
